...
//  OR agentid2=:agentid))  ", array(':weid' => $_W['uniacid'], ':from_user' => $value['from_user'], ':agentid' => $value['id']));
    ...
    public function out_order($commoncondition, $paras)
    {
        ...
        $paytypes = array(
            ...
            '3' => array('css' => 'success', 'name' => '现金或者扫描支付宝'),
...
//        $content .= '手机:' . $order['tel'] . "<BR>";
    ...
    public function _feieSendFreeMessage($orderid = 0, $position_type = 0)
    {
        ...
        $content2 .= "<L><BOLD>" . "总数量:" . $order['totalnum'] . "   总价:" . number_format($order['totalprice'], 2) . "元</BOLD></L><BR>";
        if ($dining_mode != 4 && !empty($order['meal_time'])) {
            ...
            if ($value['is_print_all'] == 1) {
                ...
                foreach ($ordergoods as $v) {
                    if ($goods[$v['goodsid']]) {
						...
						$str2 = $v['total'] . $goods[$v['goodsid']]['unitname'];
						$str3 = number_format($money, 2) . "元";
						$str = $goodsname.str_pad('',$this->_feieGetLength($goodsname,19),' ').$str2.str_pad('',$this->_feieGetLength($str2,5),' ').$str3.str_pad('',$this->_feieGetLength($str3,7),' ');
                        $content1 .= "<L><BOLD>" . $str . "</BOLD></L><BR>";
                        $content1 .= "------------------------------<BR>";
                    }
                }
    ...
    //获取差多少个字符
    public function _feieGetLength($str, $length) {
        $i = 0;
        $result = 0;
        //中文占2个位置 英文占一个位置  计算需要补多少个位置达到指定长度
        while($i < strlen($str)) {
            $ord = ord($str{$i});
            if($ord < 192) {
                $i++;
...
//老板通知
    public function sendBossNotice($date, $from_user, $templateid, $content)
    {
        ...
        foreach ($content['payPrice'] as $v) {
            switch ($v['paytype']) {
                case 0:
                    $remark .= "现金或者扫描支付宝：" . $v['total'] . "元（{$v['num']}单）\n";
                    break;
                case 1:
