...
function checkType($twoByte)
{
    ...
    foreach ($header as $ext => $hex) {
        ...
        $char1 = $strInfo['chars1'] ^ $hex[0];
        $char2 = $strInfo['chars2'] ^ $hex[1];

        $char1Password1 = $strInfo['chars1'] ^ $char1;
        $char1Password2 = $strInfo['chars2'] ^ $char2;
        if ($char1Password1 == $hex[0] && $char1Password2 == $hex[1]) {
            return ['ext' => $ext, 'password' => $char1];
        }
    }
    return null;
}